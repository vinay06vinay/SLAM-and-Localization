...
# print(p_est.shape,v_est.shape,q_est.shape)
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    R = sensor_var * np.eye(3)  # Measurement noise covariance
    ...
    p_cov_hat = np.matmul ( (np.eye(9) - K @ h_jac), p_cov_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    f_world = np.dot(C_ns_rotation,f_k_minus_1 ) + g
    p_est[k] = p_prev + (delta_t*v_prev) + ((delta_t**2) / 2) * f_world
    # print("p",p_est)
    v_est[k] = v_prev + delta_t * f_world
    q_est[k] =  Quaternion(axis_angle= w_k_minus_1 * delta_t).quat_mult_right(q_prev)

    # 1.1 Linearize the motion model and compute Jacobians
    F = np.eye(9)
    F[0:3, 3:6] = np.eye(3) * delta_t
    F[3:6, 6:9] = -skew_symmetric(C_ns_rotation @ f_k_minus_1) * delta_t

    # process noise covariance for the IMU inputs (6x6)
    ...
    if lidar_i < len(lidar.t) and lidar.t[lidar_i] <= imu_f.t[k-1]:
        ...
        v_check = v_est[k]
        q_check = q_est[k]
        y_k = lidar.data[lidar_i] # ground truth lidar measurement
